...
One-off overrides without saving anything: `--api-key`, `--base-url`, `--model`, `--api-dialect`. Environment: `KNOWTHYSELF_API_KEY`, `KNOWTHYSELF_BASE_URL`, `KNOWTHYSELF_MODEL`, `KNOWTHYSELF_API_DIALECT`, and `KNOWTHYSELF_AUTH_TOKEN` for an OAuth bearer token. Precedence is flags, then these environment variables, then the selected provider, then defaults. A bare `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` in your shell is **not** borrowed automatically — those belong to other tools; pick a provider (the Anthropic and OpenAI presets read those variables when you choose them) or set `KNOWTHYSELF_API_KEY` so nothing is used without your say-so.
...
**Before anything is sent**, a consent screen shows the endpoint, the model, the exact volume, and a token estimate, and lets you page through the precise redacted text that would leave your machine. Nothing is transmitted until you accept, and you approve every run that actually sends — consent is not remembered. Re-running over an unchanged corpus is served from the local cache and sends nothing, so it never asks; adding sessions produces a new sample and asks again before that new text leaves. There is no way to send anything without a terminal to approve it in.
