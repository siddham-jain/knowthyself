// Command reflect reads your AI coding assistant session logs and profiles how you
...
import (
	...
	"github.com/siddham/reflect/internal/engine"
	...
	"github.com/siddham/reflect/internal/report"
	"github.com/siddham/reflect/internal/store"
	"github.com/siddham/reflect/internal/tui"
)
...
func main() {
	if err := run(os.Args[1:]); err != nil {
		fmt.Fprintln(os.Stderr, "reflect: "+err.Error())
		os.Exit(1)
	}
}
...
func run(args []string) error {
	...
	if *showVersion {
		fmt.Printf("reflect %s (commit %s, built %s)\n", version, commit, date)
		return nil
	}
	...
	if *sourceID == claude.ID && !claude.Available() {
		...
		return fmt.Errorf("no Claude Code history at %s — install Claude Code (claude.com/claude-code), then run reflect",
			claude.DefaultBase())
	}
	...
	if len(sessions) == 0 {
		...
		return fmt.Errorf("no %s sessions found yet — use Claude Code a bit, then run reflect", *sourceID)
	}
	...
	if *deepEval {
		...
		if de := insight.NewDeepEval(); de != nil {
			...
			fmt.Fprintln(os.Stderr, "reflect: --deep-eval set but no ANTHROPIC_API_KEY; using heuristic tips")
		}
	}
